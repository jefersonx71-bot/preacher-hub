...
export const enrichTopic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<EnrichResult> => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
            ...
            references: ["Mateus 7:24", "Salmo 127:1", "1 Coríntios 3:11", "Lucas 6:48"],
          });
        }, 1200);
      });
    }
    ...
    return {
      illustrations: Array.isArray(parsed.illustrations)
        ? parsed.illustrations.filter(Boolean)
        : [],
...
export const expandTopicContent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ expandedContent: string }> => {
    ...
    if (!apiKey) {
      console.warn(
        "Nenhuma chave de API (GEMINI_API_KEY ou LOVABLE_API_KEY) configurada. Usando simulação local.",
      );
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          if (
            titleLower.includes("vento") ||
            titleLower.includes("mar") ||
            titleLower.includes("caminho")
          ) {
            ...
            titleLower.includes("rocha") ||
            titleLower.includes("fundamento") ||
            titleLower.includes("alicerce")
          ) {
            topicSpecificAdditions = [
            ...
            titleLower.includes("âncora") ||
            titleLower.includes("alma") ||
            titleLower.includes("tempestade") ||
            titleLower.includes("firme")
          ) {
            topicSpecificAdditions = [
          ...
          const lines = data.currentContent
            .split("\n")
            .map((l) => l.trim())
            .filter(Boolean);
    ...
    const response = await fetch(baseUrl, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Contexto do Sermão: Tema: ${data.theme || "N/A"}, Versículo: ${data.baseVerse || "N/A"}\nTópico: ${data.topicTitle}\nConteúdo Atual:\n${data.currentContent}`,
          },
        ],
        temperature: 0.7,
      }),
