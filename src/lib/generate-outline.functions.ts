...
export const generateOutline = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<GeneratedOutline> => {
    ...
    if (!apiKey) {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          if (
            lowerText.includes("bartimeu") ||
            lowerText.includes("cego") ||
            lowerText.includes("jericó") ||
            lowerText.includes("jerico")
          ) {
            resolve({
              title: "O Clamor de Bartimeu: Vencendo Barreiras para Ver Jesus",
              baseVerse: "Marcos 10:46-52",
              theme:
                "Como a persistência, o descarte de velhas vestes e o clamor de fé movem o coração de Deus.",
              introduction:
                "Às margens do caminho de Jericó, um homem cego e mendigo chamado Bartimeu clama pela intervenção de Jesus. Embora a multidão tente calá-lo, seu clamor atrai a atenção do Mestre. Hoje aprenderemos que nenhuma barreira social ou limitação física pode impedir um coração resoluto e cheio de fé de alcançar o milagre.",
              conclusion:
                "Jesus ainda passa hoje ao nosso lado. Não permita que o orgulho ou as vozes da multidão o calem. Lance fora a sua capa da autocompaixão, clame pela misericórdia de Cristo e caminhe na luz da Sua salvação.",
              appeal:
                "O Senhor está te chamando hoje. Se você está cansado de viver à beira do caminho mendigando atenção, lance fora a sua capa da autocompaixão e corra para os braços Daquele que te salva. Venha para o altar agora e declare a sua fé!",
              tags: ["Fé", "Salvação", "Oração"],
              topics: [
                {
                  title: "1. Clamando acima das vozes da multidão",
                  content:
                    "• Bartimeu não se calou quando mandaram ele silenciar (Marcos 10:48).\n• Oposição externa testa a profundidade do nosso desejo por mudança.\n• Clame com mais força: 'Jesus, Filho de Davi, tem misericórdia de mim!'.",
                },
                {
                  title: "2. Lançando fora a capa da velha identidade",
                  content:
                    "• Ele jogou fora a capa de mendigo para ir até Jesus (Marcos 10:50).\n• A capa representava seu sustento legal, sua zona de conforto e sua antiga limitação.\n• Aplicação homilética: o que você precisa abandonar hoje para responder ao chamado de Cristo?",
                },
                {
                  title: "3. Uma fé que se traduz em ação",
                  content:
                    "• Jesus pergunta: 'O que queres que eu te faça?' demonstrando a importância de especificar nossa petição.\n• O milagre restaura a visão e o transforma de um mendigo à beira do caminho em um discípulo que segue a Jesus na estrada.\n• A fé salvadora produz direção e discipulado contínuo.",
                },
              ],
            });
            return;
          ...
          resolve({
            ...
            introduction:
              "Muitas vezes nos sentimos fracos e incapazes diante dos desafios da vida. No entanto, a Palavra de Deus nos revela uma fonte inesgotável de poder e graça. Hoje aprenderemos como depender totalmente Daquele que nos fortalece.",
            conclusion:
              "Não dependa de suas próprias forças ou recursos humanos. Quando você se render a Cristo e confessar sua dependência Dele, Ele derramará o Seu poder aperfeiçoado na sua fraqueza. Busque-O hoje mesmo.",
            appeal:
    ...
    const validTags =
      parsed.tags?.filter((t) => (ALLOWED_TAGS as readonly string[]).includes(t)) ?? [];
